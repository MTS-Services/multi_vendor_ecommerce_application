        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="cart-title">{{ __('Edit latest offer') }}</h4>
                ...
                <div class="card-body">
                    <form action="{{ route('setup.latest-offer.update', encrypt($latestoffer->id)) }}"
                        method="POST" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label>{{ __('Title') }} <span class="text-danger">*</span></label>
                            <input type="text" value="{{ $latestoffer->title }}" id="title"
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            const existingFiles = {
                "#image": "{{ $latestoffer->modified_image }}",
            }
            ...
            if (`{{ $latestoffer?->parent_id && $latestoffer?->parent?->parent_id }}`) {
                getLatestOffer(`{{ $latestoffer?->parent?->parent_id }}`, route,
                    `{{ $latestoffer?->parent_id }}`);
            }
        });
    </script>
    {{-- FilePond  --}}
